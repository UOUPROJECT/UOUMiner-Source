...
extern int scanhash_pipe(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done,int blocklen);

/* free device allocated memory per algo */
void algo_free_all(int thr_id);

extern void free_pipe(int thr_id);
...
void pipe_hash(unsigned char* output, const unsigned char* input, int len);
#ifdef __cplusplus
}
#endif
