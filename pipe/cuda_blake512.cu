...
__host__ void blake512_cpu_hash(int thr_id, uint32_t threads, uint32_t startNounce)
{
	...
	if (BLOCKSIZE == 80)
		blake512_gpu_hash<80><<<grid, block, shared_size>>>(threads, startNounce, d_hash5output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
	else if (BLOCKSIZE == 84)
		blake512_gpu_hash<84><<<grid, block, shared_size>>>(threads, startNounce, d_hash5output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
}
