...
#pragma unroll 8
	...
	// the macro sets each 4 bits from four different hashes to a nibble together
...
{
	// these copies are optional, as the hashes are already on the gpu should lie

	const uint32_t threadsperblock = 128;

	// calculate how many thread blocks we need
	dim3 grid((threads + threadsperblock-1)/threadsperblock);
	dim3 block(threadsperblock);

	combine_gpu_hash <<<grid, block>>> (threads, startNounce, d_hashoutput[thr_id], d_hash2output[thr_id], d_hash3output[thr_id], d_hash4output[thr_id], d_hash5output[thr_id], pipe_nonceVector[thr_id]);
