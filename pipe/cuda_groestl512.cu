...
extern uint32_t *pipe_nonceVector[MAX_GPUS];
...
__host__ void groestl512_cpu_copyHeftyHash(int thr_id, uint32_t threads, void *heftyHashes, int copy)
{
	...
	if (copy)
		CUDA_SAFE_CALL(cudaMemcpy(pipe_heftyHashes[thr_id], heftyHashes, 8 * sizeof(uint32_t) * threads, cudaMemcpyHostToDevice));
}
...
__host__ void groestl512_cpu_hash(int thr_id, uint32_t threads, uint32_t startNounce)
{
	...
	if (BLOCKSIZE == 84)
		groestl512_gpu_hash<84><<<grid, block, shared_size>>>(threads, startNounce, d_hash4output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
	else if (BLOCKSIZE == 80)
		groestl512_gpu_hash<80><<<grid, block, shared_size>>>(threads, startNounce, d_hash4output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
}
