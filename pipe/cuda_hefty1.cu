...
uint32_t *pipe_heftyHashes[MAX_GPUS];
...
void hefty_cpu_init(int thr_id, uint32_t threads)
{
    ...
    CUDA_SAFE_CALL(cudaMalloc(&pipe_heftyHashes[thr_id], (size_t) 32 * threads));
}

__host__
void hefty_cpu_free(int thr_id)
{
    cudaFree(pipe_heftyHashes[thr_id]);
}
...
// data muss 80/84-Byte haben!
{
	...
	//the first round will be carried out on the cpu for all threads is equal.the hash is then passed to the thread

    // create expanded block w
    uint32_t W[64];
    memcpy(W, &msgBlock[0], sizeof(uint32_t) * 16);
    for(int j=16;j<64;j++)
        W[j] = s1(W[j-2]) + W[j-7] + s0(W[j-15]) + W[j-16];

    // initiating register a to h with the hash table
    uint32_t regs[8];
    uint32_t hash[8];
    uint32_t sponge[4];
...
void hefty_cpu_hash(int thr_id, uint32_t threads, int startNounce)
{
    ...
    hefty_gpu_hash <<< grid, block, shared_size >>> (threads, startNounce, pipe_heftyHashes[thr_id]);
...
void hefty_copy_hashes(int thr_id, uint32_t threads, uint32_t* d_outputhash)
{
    ...
    dim3 grid((threads + threadsperblock - 1) / threadsperblock);
    dim3 block(threadsperblock);
    hefty_gpu_copy <<< grid, block >>> (threads, pipe_heftyHashes[thr_id], (uint64_t*) d_outputhash);
