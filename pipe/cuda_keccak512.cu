...
void keccak512_cpu_hash(int thr_id, uint32_t threads, uint32_t startNounce)
{
	...
	if (BLOCKSIZE==84)
		keccak512_gpu_hash<84><<<grid, block, shared_size>>>(threads, startNounce, d_hash3output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
	else if (BLOCKSIZE==80)
		keccak512_gpu_hash<80><<<grid, block, shared_size>>>(threads, startNounce, d_hash3output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
}
