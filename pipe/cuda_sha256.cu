...
__host__ void sha256_cpu_hash(int thr_id, uint32_t threads, int startNounce)
{
	...
	if (BLOCKSIZE == 84)
		sha256_gpu_hash<84><<<grid, block, shared_size>>>(threads, startNounce, d_hash2output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
	else if (BLOCKSIZE == 80) {
		sha256_gpu_hash<80><<<grid, block, shared_size>>>(threads, startNounce, d_hash2output[thr_id], pipe_heftyHashes[thr_id], pipe_nonceVector[thr_id]);
	}
}
