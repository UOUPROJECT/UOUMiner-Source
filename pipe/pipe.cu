...
#warning "Pipe: incompatible GCC version!"
#define USE_THRUST 0
#else
#define USE_THRUST 1
#endif
...
uint32_t *pipe_nonceVector[MAX_GPUS];
...
extern uint32_t *pipe_heftyHashes[MAX_GPUS];
...
__host__
int scanhash_pipe(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done, int blocklen)
{
	...
	// create an individual target for each hash based on the highest bit that is set in ptarget.
	...
	if (!init[thr_id])
	{
		...
		CUDA_SAFE_CALL(cudaMalloc(&pipe_nonceVector[thr_id], sizeof(uint32_t) * throughput));
	...
	do {
...
#if USE_THRUST
		thrust::device_ptr<uint32_t> devNoncePtr(pipe_nonceVector[thr_id]);
		thrust::device_ptr<uint32_t> devNoncePtrEnd((pipe_nonceVector[thr_id]) + throughput);
		...
		if(actualNumberOfValuesInNonceVectorGPU > 0)
		{
			...
			cudaMemcpy(cpu_nonceVector, pipe_nonceVector[thr_id], size, cudaMemcpyDeviceToHost);
			...
			for (uint32_t i=0; i < actualNumberOfValuesInNonceVectorGPU; i++)
			{
				...
				if (foundhash[7] <= ptarget[7] && fulltest(foundhash, ptarget)) {
					...
					pipe_hash((uchar*)vhash, (uchar*)pdata, blocklen);
...
// cleanup
extern "C" void free_pipe(int thr_id)
{
	if (!init[thr_id])
		return;
	...
	cudaFree(pipe_nonceVector[thr_id]);
...
// CPU hash function
__host__
void pipe_hash(uchar* output, const uchar* input, int len)
{
