...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	switch (opt_algo) {
	case ALGO_PIPE:
			//pipe_hash(merkle_root, sctx->job.coinbase, (int)sctx->job.coinbase_size);
	...
	for (i = 0; i < sctx->job.merkle_count; i++) {
		...
		//if (opt_algo == ALGO_PIPE || opt_algo == ALGO_MJOLLNIR)
		//	pipe_hash(merkle_root, merkle_root, 64);
		//else
...
static void *miner_thread(void *userdata)
{
	...
	while (!abort_flag) {
		...
		switch (opt_algo) {
		case ALGO_PIPE:
			rc = scanhash_pipe(thr_id, &work, max_nonce, &hashes_done, MNR_BLKHDR_SZ);
			break;
		default:
			/* should never happen */
			goto out;
...
int main(int argc, char *argv[])
{
	...
	printf("*** uouminer " PACKAGE_VERSION " for nVidia GPUs by UOUCOIN developers ***\n");
	if (!opt_quiet) {
