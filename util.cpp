...
void get_defconfig_path(char *out, size_t bufsize, char *argv0)
{
	...
	struct stat info;
#ifdef WIN32
	snprintf(out, bufsize, "%s\\uouminer\\uouminer.conf\0", getenv("APPDATA"));
#else
	snprintf(out, bufsize, "%s\\.uouminer\\uouminer.conf", getenv("HOME"));
#endif
	if (dir && stat(out, &info) != 0) {
		// binary folder if not present in user folder
		snprintf(out, bufsize, "%s%uouminer.conf%s", dir, sep, "");
	}
	if (stat(out, &info) != 0) {
		out[0] = '\0';
		return;
